...
clean-pyc:
	...
	@find bodhisattva -name '*.pyc' -exec rm -f {} + || true
	@find bodhisattva -name '*.pyo' -exec rm -f {} + || true
	@find bodhisattva -name '__pycache__' -exec rm -fr {} + || true
...
test: clean .build_history/pylint .build_history/bandit poetry.lock
	...
	$(VENV) pytest --doctest-modules bodhisattva
	$(VENV) python -m unittest discover
	$(VENV) py.test tests --cov=bodhisattva --cov-report=html --cov-fail-under 50
...
.build_history/black: .build_history .build_history/isort $(FILES)
	...
	$(VENV) black bodhisattva --exclude .venv
...
.build_history/pylint: .build_history .build_history/isort .build_history/black $(FILES)
	...
	$(VENV) ruff --fix
	$(VENV) pylint bodhisattva --fail-under 9.7
...
.PHONY: mypy
mypy:
	$(VENV) mypy bodhisattva --ignore-missing-imports --check-untyped-defs

.PHONY:
docker:
	docker build -t bodhisattva -f Dockerfile .


.PHONY: gen_code
gen_code:
	echo "Should check mypy and docstrings first."
	cd bodhisattva && cd code_generate && python generate_schema.py
	cd bodhisattva && cd code_generate && python generate_cli.py
	cd bodhisattva && cd code_generate && python generate_toolkit.py
	pwd

check_docs:
	$(VENV) interrogate bodhisattva --verbose
	$(VENV) pydoctest --config .pydoctest.json | grep -v "__init__" | grep -v "__main__" | grep -v "Unable to parse"

make_docs:
	pdoc bodhisattva --html -o docs --force
...
check_spelling:
	$(VENV) pylint bodhisattva --enable C0402 --rcfile=.pylintrc_spell
	$(VENV) codespell README.md --ignore-words=private_dictionary.txt
	$(VENV) codespell bodhisattva --ignore-words=private_dictionary.txt
